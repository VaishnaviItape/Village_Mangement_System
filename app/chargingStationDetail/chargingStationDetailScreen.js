...
const TenantScreen = () => {
  ...
  const { tenantId, unitId: paramUnitId } = route.params || {};
  const [savedImages, setSavedImages] = useState([]);   // Existing tenant images from API
  const [tenantImages, setTenantImages] = useState([]); // Newly picked images
  ...
  const [tenantAgreements, setTenantAgreements] = useState([]); // New uploads
  const [panCard, setPanCard] = useState([]);       // Array to allow multiple uploads if needed
  ...
  const pickAgreement = async () => {
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: "images", // allow images and PDFs
      allowsEditing: false,
      quality: 1,
    });
      ...
      const fetchTenant = async () => {
        try {
          ...
          if (tenant) {
            ...
            setMoveOutDate(tenant.moveOutDate?.slice(0, 10) || "");
  ...
  const validateDates = () => {
    ...
    if (!moveInDate) { newErrors.moveInDate = "Move-in date is required"; valid = false; }
    if (!moveOutDate) { newErrors.moveOutDate = "Move-out date is required"; valid = false; }
    else if (moveOutDate < moveInDate) { newErrors.moveOutDate = "Move-out date cannot be before Move-in date"; valid = false; }
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    const tenantData = { fullName, email, phoneNumber, age: parseInt(age), occupation, moveInDate, moveOutDate, unitId, tenantType };
    ...
    try {
      ...
      if (isEditMode) {
        ...
        // Upload tenant images if any
        if (tenantImages.length > 0) {
          await uploadAttachment(tenantImages, tenantId, "Tenant", token);
        }
        // Upload PAN Card and Aadhaar Card using the same API and entityType "Tenant"
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 80 : 0}>
        <ScrollView contentContainerStyle={styles.scrollViewContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View style={styles.headerContainer}>
            ...
            <Text style={styles.headerTitle}> <Ionicons name="create-outline" size={16} color="#fff" />{isEditMode ? "Edit Tenant" : "Add Tenant"}</Text>
          </View>
          ...
          <View style={styles.formCard}>
            ...
            <TouchableOpacity style={[styles.input, errors.moveInDate ? styles.inputError : {}]} onPress={() => setShowMoveInPicker(true)}>
              <Text style={{ color: moveInDate ? "#000" : Colors.grayColor }}>{moveInDate || "Select Move-in Date"}</Text>
            </TouchableOpacity>
            {showMoveInPicker && (
              <DateTimePicker
            ...
            <TouchableOpacity style={[styles.input, errors.moveOutDate ? styles.inputError : {}]} onPress={() => setShowMoveOutPicker(true)}>
              <Text style={{ color: moveOutDate ? "#000" : Colors.grayColor }}>{moveOutDate || "Select Move-out Date"}</Text>
            </TouchableOpacity>
            {showMoveOutPicker && (
              <DateTimePicker
